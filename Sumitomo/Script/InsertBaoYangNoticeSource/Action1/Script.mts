...
Datatable("ObjParts_5_8","Action1")=year(InfoGeneTime) &"-"& right("0"& month(InfoGeneTime),2) &"-"& right("0"& day(InfoGeneTime),2)
else
InfoGeneTimeHex=right("0"& hex(right(year(InfoGeneTime),2)),2) & right("0"& hex(month(InfoGeneTime)),2) & right("0"& hex(day(date+1)),2) _
&right("0"& hex(hour(InfoGeneTime)),2) & right("0"& hex(minute(InfoGeneTime)),2) & right("0"& hex(second(InfoGeneTime)),2)   '当前时间十六进制形式
Datatable("ObjParts_7_8","Action1")=year(InfoGeneTime) &"-"& right("0"& month(InfoGeneTime),2) &"-"& right("0"& day(date+1),2)
end if
'========定义并获取源码其它内容
Dim OtherContent
