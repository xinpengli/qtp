...
Dim MsgHeadPart 
MsgHeadPart=Datatable("TermPara2_MsgHeadPart","Global")
'========定义并获取信息生成时间
Dim InfoGeneTime 
InfoGeneTime=Cstr(now)  '获取当前时间
Datatable("TermPara2_InfoGeneTime","Global")=InfoGeneTime '存储当前时间的十进制形式,用于后续页面相关时间检查
...
'========获取对照码
Dim sqlQuery          '获取数据库查询语句
sqlQuery="select MsgTS2_CmdSign from sumitomo.dbo.Msg_Tmnl_Set2 where MsgTS2_Vcl_ID=(select Vcl_ID from Sumitomo.dbo.VclInfo where Vcl_No='"&Datatable("Vcl_No","Global")&"')"
ControlCode=QueryDBColumn(sqlQuery,"MsgTS2_CmdSign")
'========定义并获取源码其它内容
Dim OtherContent
OtherContent=Datatable("TermPara2_OtherContent","Global")
'========定义并获取检验码
Dim TempStr
TempStr=MsgHeadPart & InfoGeneTimeHex & ControlCode & OtherContent
'定义和获取 校验码和结束符
