	...
	For i=1 to wt.ColumnCount(1)
		...
		Select Case ColName
			Case "信息发送时间":'信息发送时间ExpLockRep_MsgSendTime 等同 源码中信息生成时间LockSetSour_InfoGeneTime，此处只是对应页面列出
				...
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("LockSetRepSour_InfoGeneTime","Global")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("LockSetRepSour_InfoGeneTime","Global")&" 实际值："&ColVal
				end if				
			Case "立即锁":
				if(ColVal=Datatable("ExpLockSetRep_ImmeLock","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_ImmeLock","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_ImmeLock","Action1")&" 实际值："&ColVal
				end if
			Case "总工作时间锁":
				if(ColVal=Datatable("ExpLockSetRep_WorkTimeLock","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_WorkTimeLock","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_WorkTimeLock","Action1")&" 实际值："&ColVal
				end if
			Case "工作时间":
				if(ColVal=Datatable("ExpLockSetRep_WorkTimeLock_Hour","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_WorkTimeLock_Hour","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_WorkTimeLock_Hour","Action1")&" 实际值："&ColVal
				end if
			Case "指定日期锁": 
				if(ColVal=Datatable("ExpLockSetRep_AppDateLock","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppDateLock","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppDateLock","Action1")&" 实际值："&ColVal
				end if
			Case "指定日期":'检查不使用Action1中的ExpLockSetRep_AppDateLock，使用Global中的AppDateLock_Date
				...
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("AppDateLock_Date","Global")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("AppDateLock_Date","Global")&" 实际值："&ColVal
				end if
			Case "位置锁":
				if(ColVal=Datatable("ExpLockSetRep_AppPosiLock","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock","Action1")&" 实际值："&ColVal
				end if
			Case "经度":
				if(ColVal=Datatable("ExpLockSetRep_AppPosiLock_Long","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Long","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Long","Action1")&" 实际值："&ColVal
				end if
			Case "纬度":
				if(ColVal=Datatable("ExpLockSetRep_AppPosiLock_Lati","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Lati","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Lati","Action1")&" 实际值："&ColVal
				end if
			Case "半径":
				if(ColVal=Datatable("ExpLockSetRep_AppPosiLock_Radi","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Radi","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_AppPosiLock_Radi","Action1")&" 实际值："&ColVal
				end if
			Case "循环密码锁":
				if(ColVal=Datatable("ExpLockSetRep_CircDateLock","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_CircDateLock","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_CircDateLock","Action1")&" 实际值："&ColVal
				end if
			Case "循环密码锁时间":
				...
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_CircDateLock_Date","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_CircDateLock_Date","Action1")&" 实际值："&ColVal
				end if
			Case "总工作时间":
				if(ColVal=Datatable("ExpLockSetRep_ToalWorkHour5","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_ToalWorkHour5","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_ToalWorkHour5","Action1")&" 实际值："&ColVal
				end if
'			Case "经度":
'				if(ColVal=Datatable("ExpLockSetRep_Long5","Action1"))then
'				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"5检查","期望值："&Datatable("ExpLockSetRep_Long5","Action1")&" 实际值："&ColVal
'				else
'				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"5检查","期望值："&Datatable("ExpLockSetRep_Long5","Action1")&" 实际值："&ColVal
'				end if
'			Case "纬度":
'				if(ColVal=Datatable("ExpLockSetRep_lati5","Action1"))then
'				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"5检查","期望值："&Datatable("ExpLockSetRep_lati5","Action1")&" 实际值："&ColVal
'				else
'				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"5检查","期望值："&Datatable("ExpLockSetRep_lati5","Action1")&" 实际值："&ColVal
'				end if
			Case "位置":
				if(ColVal=Datatable("ExpLockSetRep_Position5","Action1"))then
				reporter.ReportEvent micPass,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_Position5","Action1")&" 实际值："&ColVal
				else
				reporter.ReportEvent micFail,"锁车设置回复列表-"&ColName&"检查","期望值："&Datatable("ExpLockSetRep_Position5","Action1")&" 实际值："&ColVal
				end if
		End Select
	Next
	'========下载功能
	...
	For i=1 to ColCount
		...
		if(trim(wt.GetCellData(1,i))=oExcel.Worksheets(1).cells(1,i))then
			...
			reporter.ReportEvent micFail,"Excel列-"& oExcel.Worksheets(1).cells(1,i)&"-检查失败","期望值："&wt.GetCellData(2,i)&" 实际值："& oExcel.Worksheets(1).cells(2,i)
			end if
		end if
	Next
	' 关闭工作簿        
