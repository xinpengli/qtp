...
if(i<5)then
	...
	'插入第5组源码时，即保养通知信息(复位场景的)，使用如下信息生成时间，区别于前4组的是信息生成时间需要比保养所属日期晚一天
	InfoGeneTimeHex=right("0"& hex(right(year(InfoGeneTime),2)),2) & right("0"& hex(month(InfoGeneTime)),2) & right("0"& hex(day(date+1)),2) _
	&right("0"& hex(hour(InfoGeneTime)),2) & right("0"& hex(minute(InfoGeneTime)),2) & right("0"& hex(second(InfoGeneTime)),2)   '当前时间十六进制形式
end if
'如果插入的是第一组源码即保养时间设定回复的,则需要获取对照码
if(i=1)then
'Datatable("ControlCode1","Global")
end if
'========定义并获取源码其它内容
Dim OtherContent
OtherContent=Datatable("OtherContent"&i,"Global")
'如果源码为保养信息，则需要在内容串前加上保养所属日期
if(mid(MsgHeadPart,3,2)="26")then
	Dim BaoYangDate
	BaoYangDate=right("0"&day(date),2) & right("0"& month(date),2) & right(year(date),2)
	OtherContent=BaoYangDate & OtherContent
end if
'========定义并获取检验码
Dim TempStr
if(i<>1)then
...
TempStr=MsgHeadPart  & InfoGeneTimeHex & Datatable("ControlCode1","Global") & OtherContent
end if
'定义和获取 校验码和结束符
