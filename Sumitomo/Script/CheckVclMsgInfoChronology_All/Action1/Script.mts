...
if(Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Exist)then
	Select Case i
		Case "1":
			Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Select  "6" 
		Case "2":
			Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Select  "5" 
		Case "3":
			Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Select  "4" 
		Case "4":
			Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Select  "3" 
		Case "5":
			Browser("住友").Page("主页_车辆信息").Frame("信息年表").WebRadioGroup("查询项按钮组").Select  "8" 
	End Select
end if
'========选择信息年表查看的年份
...
'分别获取行列,mm对应行,dd对应列
Dim mm,dd
mm=month(Datatable("InfoGeneTime"&i,"Global"))
dd=day(Datatable("InfoGeneTime"&i,"Global"))
Dim ActVal,ExpVal  '定义实际值和期望值
'获取实际值,如果是日志的信息强度的话,需要定位"天"早于源码信息生成时间一天,其它源码的检查日期同其信息生成时间
if(mid(Datatable("MsgHeadPart"&i,"Global"),3,2)<>"20")then
	ActVal=trim(InfoChro.GetCellData(mm,dd))
else
	dd=day(Cdate(Datatable("InfoGeneTime"&i,"Global"))-1)
	ActVal=trim(InfoChro.GetCellData(mm,dd))
end if
'获取期望值
ExpVal=Datatable("ExpInfoChroVal"&i,"Action1")
if(ActVal=ExpVal)then
reporter.ReportEvent micPass,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&"信息年表-"&mid(Datatable("MsgHeadPart"&i,"Global"),3,2)&"指令"&Datatable("StatTaskExeDate","Global")&"统计后检查通过","期望值: "& ExpVal &" 实际值:"& ActVal
else
reporter.ReportEvent micFail,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&"信息年表-"&mid(Datatable("MsgHeadPart"&i,"Global"),3,2)&"指令"&Datatable("StatTaskExeDate","Global")&"统计后检查失败","期望值: "& ExpVal &" 实际值:"& ActVal
end if
'========返回车辆信息页
