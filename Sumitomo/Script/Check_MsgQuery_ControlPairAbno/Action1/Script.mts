...
if(Browser("住友").Page("主页_车辆信息").Frame("信息查询").WebTable("信息查询列表").Exist)then
	...
	For i=1 to wt.ColumnCount(1)
		...
		Select Case ColName
			...
			Case "配对未通过状态":
				if(ColVal=Datatable("PairNotPassStatus","Action1"))then
				reporter.ReportEvent micPass,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&Datatable("MsgType","Global")&"-"&ColName&"-检查通过","期望值: "& Datatable("PairNotPassStatus","Action1") &"实际值: "& ColVal
				else
				reporter.ReportEvent micFail,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&Datatable("MsgType","Global")&"-"&ColName&"-检查通过","期望值: "& Datatable("PairNotPassStatus","Action1") &"实际值: "& ColVal
				end if
		End Select
	Next
	'下载文件 
