...
SqlDel="delete Sumitomo.dbo.Msg_Timing_"&TempDate&" where msgt_vcl_id=(select Vcl_ID from Sumitomo.dbo.VclInfo where Vcl_No='"&Datatable("Vcl_No","Global")&"');delete Sumitomo.dbo.Msg_Timing_Last where MsgTL_Vcl_ID=(select Vcl_ID from Sumitomo.dbo.VclInfo where Vcl_No='"&Datatable("Vcl_No","Global")&"');delete Sumitomo.dbo.Msg_PairFailed  where MsgPF_Vcl_ID=(select Vcl_ID from Sumitomo.dbo.VclInfo where Vcl_No='"&Datatable("Vcl_No","Global")&"');delete Sumitomo.dbo.Msg_PairFailed_Last where MsgPFL_Vcl_ID=(select Vcl_ID from Sumitomo.dbo.VclInfo where Vcl_No='"&Datatable("Vcl_No","Global")&"')"
'执行sql并返回结果
Dim RetuVal
RetuVal=ExecDB(SqlDel) 
'根据执行结果写日志
if(RetuVal>=0)then
reporter.ReportEvent micPass,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&"设备"&Datatable("Vcl_No","Global")&"匹配状态数据清除完毕","设备"&Datatable("Vcl_No","Global")&"匹配状态数据清除完毕"
else
reporter.ReportEvent micFail,Environment("TestName")&"?"&Datatable.GetSheet("Global").GetCurrentRow&"?"&"设备"&Datatable("Vcl_No","Global")&"匹配状态数据清除完毕","设备"&Datatable("Vcl_No","Global")&"匹配状态数据清除完毕"
end if
'记录err
