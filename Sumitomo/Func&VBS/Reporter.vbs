...
'***************************************************************************************************************************
    ...
    Public Sub ReportEvent(iStatus, sStepName, sDetails) 
		...
		ExcelPath=mfl.Locate("Sumitomo") & "\Result\Reporter"&Cstr(date())&".xls"
		TxtPath=mfl.Locate("Sumitomo") & "\Result\ExcelCurrRow"&Cstr(date())&".txt"
        StatusTxtPath=mfl.Locate("Sumitomo") & "\Result\CaseStatus"&Cstr(date())&".txt"
    ...
    Private Sub Class_Initialize
        ...
        ' 创建mfl对象
        Set mfl = CreateObject("Mercury.FileLocator")      
    End Sub 
    ...
    Sub  XmlDomDoc_ErrLog(caseName,versionNo,actionName,currRow,rowCount,errCode,errDesc,errSour,RecTime)  
        ...
        Dim xmPath
		xmlPath=mfl.Locate("Sumitomo") & "\Result\ErrLog"&Cstr(date())&".xml"
        '定义xml文档对象
		Dim xmlDoc     
